...
2. Run `python3 scripts/generate_email.py demo/input/personalized-outreach.json` from the repository root for the jury demo. For other local inputs, replace only the final path argument.
3. Preserve the program's JSON result exactly. A successful result contains `subject`, `email_body`, `evidence_ledger`, and `limitations`.
4. Treat exit code `0` as success, `2` as malformed or source-invalid input, and `3` as insufficient usable evidence or an unsatisfied word limit. For any nonzero exit, return the failure JSON and do not draft an alternative.
...
- In `evidence_ledger`, identify the exact fact used, its exact source URL, its category, and its `[1]` marker.
