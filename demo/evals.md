...
| --- | --- | --- | --- | --- | --- |
| Intended | `demo/input/personalized-outreach.json` (`company_url: https://cal.com`, live scrape, sender intentionally unset) | The skill asks once for sender name/role/offer/call-to-action, `scripts/scrape_company.py` returns a sourced fact, then `scripts/generate_email.py` produces a grounded email with subject, body, evidence ledger, and claim audit; exit code 0 | Ran the full pipeline end to end: the scraper returned a genuine, sourced fact from `cal.com`, sender details were supplied (the presenter's own, as the skill asks), and the generator produced a subject, body, evidence ledger, and claim audit citing it; exited 0 | Pass | `demo/output/personalized-outreach.json` (captured from this exact run); generator-level cases also pass in `evals/run_evals.py` → `PASS success_grounded_draft (exit 0)` and `PASS sender_completion_flow (exit 0)` |
